...
export default function Sidebar({ collapsed }) {
  ...
  return (
    <aside
      className={`flex h-full flex-col bg-gradient-to-b from-ink to-[#0a1f52] text-white transition-all duration-200 ${
        collapsed ? 'w-20' : 'w-74'
      }`}
    >
      {/* Logo */}
      <div className="flex h-16 items-center gap-2.5 border-b border-white/10 px-5">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-400 shadow-lg shadow-brand-400/20">
          <Car size={20} className="text-ink" />
        </div>
        {!collapsed && (
          <span className="text-lg font-bold tracking-tight text-white">
            EZRide <span className="text-brand-400">Admin</span>
          </span>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {groups.map((group) => (
          <div key={group.name} className="mb-5 last:mb-0">
            {!collapsed && (
              <p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-wider text-white/35">{group.name}</p>
            )}
      ...
      <div className="border-t border-white/10 p-3">
        <div className={`flex items-center gap-3 rounded-xl bg-white/5 ${collapsed ? 'justify-center p-2' : 'p-3'}`}>
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-400 text-xs font-bold text-ink">
            {initials(user?.name)}
          </span>
          {!collapsed && (
        ...
        {!collapsed && <p className="mt-2 px-1 text-[11px] text-white/30">EZRide Admin · v1.0</p>}
      </div>
    </aside>
  )
}
